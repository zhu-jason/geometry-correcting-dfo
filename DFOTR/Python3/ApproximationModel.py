...
class ApproximationModel:
    def __init__(self, n, options):
        ...
        if self.type['model'] == 'linear':
            self.c = np.empty(1)
            self.g = np.empty(n)
        elif self.type['model'] == 'quadratic':
            self.c = np.empty(1)
            self.g = np.empty(n)
            self.H = np.empty((n,n))
    ...
    def fit(self, samp):
        ...
        elif (self.type['model'] == 'quadratic') and (m == n + 1):
    ...
    def minimize(self):
        ...
        if self.type['model'] == 'linear' or (self.type['model'] == 'quadratic' \
             and (np.isnan(self.H).any() or np.linalg.norm(self.H) < 1e-12)):
            s = - self.delta / np.linalg.norm(self.g) * self.g
            val = sum(self.g * s)

        elif (self.type['model'] == 'quadratic') and (self.type['TRsub'] == 'exact'):
            s, val = trust_sub_exact(self.H, self.g, self.delta)

        elif (self.type['model'] == 'quadratic') and (self.type['TRsub'] == 'CG'):
