...
class Sample:
    def __init__(self, x, options):
        ...
        print(Q)
        Y = x + options['tr_delta'] * Q.T
        num_z_points = (self.n + 1) * self.n // 2
        random_vectors = np.random.randn(num_z_points, self.n)
        norms = np.linalg.norm(random_vectors, axis=1, keepdims=True)
        unit_vectors = random_vectors / norms
        Z_pts = x + options['tr_delta'] * unit_vectors
        self.Y = np.concatenate((Y, Z_pts, x.reshape(1,self.n)))
